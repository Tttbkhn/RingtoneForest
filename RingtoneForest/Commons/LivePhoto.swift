...
class LivePhoto {
    ...
    public class func saveToLibrary(_ resources: LivePhotoResources, completion: @escaping (Bool) -> Void) {
        print(FileManager.default.fileExists(atPath: resources.pairedVideo.path))
        print(FileManager.default.fileExists(atPath: resources.pairedImage.path))
        
        PHPhotoLibrary.shared().performChanges({
            let creationRequest = PHAssetCreationRequest.forAsset()
            let options = PHAssetResourceCreationOptions()
            print(resources.pairedImage)
            print(resources.pairedVideo)
